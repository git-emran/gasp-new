        ...
        <div className="grid grid-cols-12 px-7">
          <div className="col-span-12 md:col-span-11 lg:col-span-7 md:col-span-9 max-w-screen-lg pb-5 leading-relaxed">
            ...
            {notes.map((note) => (
              <div
                key={note.slug}
                className="mb-16 xl:mr-60 lg:mr-30 md:mr-10 max-w-prose bg-white text-black dark:bg-black dark:text-white"
              >
                <Link
                  href={note.href}
                  target={note.isExternal ? "_blank" : undefined}
                  rel={note.isExternal ? "noopener noreferrer" : undefined}
                  className="block group post-link"
                >
                  <div className="w-full">
                    ...
                    <div className="font-medium text-black block mb-2 dark:text-white dark:hover:text-gray-300">
                      <span className="post-title text-xl">{note.title}</span>
                    </div>
                    <p className="text-black mb-4 mt-0 dark:text-gray-200">
                      {note.excerpt}
                    </p>
                  </div>
                </Link>
